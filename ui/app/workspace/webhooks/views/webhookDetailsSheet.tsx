...
export function WebhookDetailsSheet({ endpoint, isTesting, testCooldown, canManage, onTest, onClose }: WebhookDetailsSheetProps) {
	const open = !!endpoint;
					...
					<Table>
						...
						<TableBody>
							{isLoading ? (
								...
								<TableRow>
									<TableCell colSpan={8} className="text-destructive h-24 text-center" data-testid="webhook-delivery-history-error">
								...
								runs.map((run) => {
									...
									return (
										<Fragment key={run.key}>
											<TableRow data-testid={`webhook-delivery-row-${run.key}`}>
												<TableCell>
													{run.attempts.length > 1 && (
														<button
															...
															aria-label={expanded ? "Collapse delivery attempts" : "Expand delivery attempts"}
															data-testid={`webhook-delivery-expand-${run.key}`}
														>
