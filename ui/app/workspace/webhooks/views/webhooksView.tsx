							...
							<TableBody>
								{endpoints.length === 0 ? (
									...
									endpoints.map((endpoint) => (
										<TableRow key={endpoint.id} className="group" data-testid={`webhook-row-${endpoint.name}`}>
											<TableCell className="font-medium">
												<button
													type="button"
													className="text-left hover:underline"
													onClick={() => setDetailsEndpoint(endpoint)}
													data-testid={`webhook-row-open-${endpoint.name}`}
												>
													{endpoint.name}
												</button>
											</TableCell>
											...
											<TableCell className={`bg-card group-hover:bg-muted/50 sticky right-0 z-10 text-right ${PIN_SHADOW_RIGHT}`}>
												<WebhookActionsMenu
													endpoint={endpoint}
