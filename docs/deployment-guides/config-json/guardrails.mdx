...
For block-capable streaming output rules, `stream_replay_duration_ms` controls the total time over which an allowed buffered stream is replayed to the client. It defaults to `200`; set it to `0` to send all buffered events immediately. When multiple matched block-capable rules specify different values, Bifrost uses the largest value.
