...
- If any matched rule can block, Bifrost holds the complete stream until generation and guardrail evaluation finish. An allowed stream is then replayed over the rule's `stream_replay_duration_ms`; a blocked stream returns the guardrail intervention instead.

The replay duration is the total delivery window, not a delay applied to every event. It defaults to `200` milliseconds, while `0` sends all buffered events immediately. If multiple matched block-capable rules configure different durations, Bifrost uses the largest value. This behavior applies to streaming Chat Completions, Text Completions, and Responses API requests.
