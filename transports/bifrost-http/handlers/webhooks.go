...
func (h *WebhookHandler) reserveTestSlot(endpointID string) (time.Duration, bool) {
	...
	// Evict entries whose cooldown has lapsed so the map stays bounded by the
	// endpoints tested within one cooldown window rather than every endpoint
	// ever tested.
	for id, last := range h.lastTestFire {
		if now.Sub(last) >= webhookTestCooldown {
			delete(h.lastTestFire, id)
		}
	}
